...
class AIModel:
    ...
    def __init__(self, model_dir: Optional[str] = None):
        if model_dir is None:
            api_default_dir = Path(__file__).parent / "ai" / "ai_model"
            workspace_model_dir = Path(__file__).resolve().parent.parent / "model"
            if (workspace_model_dir / "model_config.json").exists():
                self.model_dir = workspace_model_dir  # type: ignore
            else:
                self.model_dir = api_default_dir  # type: ignore
        else:
    ...
    def predict(self, prediction_confs: List[PredictionConfig]) -> OutcomeProbs:
        ...
        if len(prediction_confs) > 1:
            print(
                "Warning: Multiple prediction configurations provided. Only the first one will be used."
            )
        ...
        with torch.no_grad():
            if self.model is None:
                raise ValueError(
                    "Model is not loaded. Please load the model before prediction."
                )
            ...
            outcome_dict = {
                name: cat_probs[0, i].item()
                for i, name in enumerate(categorical_outcomes)
            }
            outcome_dict.update(
                {name: bin_probs[0, j].item() for j, name in enumerate(binary_outcomes)}
            )
